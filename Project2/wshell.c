...
int main(int argc, char *argv[]) 
{
    ...
    { 
        ...
        //looks at each ID in running_process_ID. If it is evaluated as done
        // it does the propper print statemnt and updates the value at that index
        ...
        //simplifying assumption; those with the cmd & will never be && in :D
        ...
        //once user has put something in process it; put it into an array list 'commands'

        //have a function for each command with the appropriate arguments
...
void fix_process_string(char* process_stirng, char fixed_string[MAX_BUFFER]){
    ...
    //code to pick out relevant index; 
...
void run_as_background_process(char* arguments[MAX_ARGUMENTS], int* task_number, int running_process_ID[255], char* running_in_background[255]){
    ...
    //facen a string and add it to the list of strings and the relevant index
    // also add this index to the 
    itoa(*task_number);
...
// function assumes the array is "" terminated
int get_array_size(char* arguments[MAX_ARGUMENTS]){
    int i = 0;
